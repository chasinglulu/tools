...
#define SIGI_VIRT_PART_A_ACPU_TYPE ARM_CPU_TYPE_NAME("cortex-a78ae")

static bool sigi_virt_get_virt(Object *obj, Error **errp)
{
    SigiVirtPartitionA *s = SIGI_VIRT_PART_A(obj);
...
static void create_uart(SigiVirtPartitionA *s, int uart)
{
...
static void create_its(SigiVirtPartitionA *s)
{
...
static void create_gic(SigiVirtPartitionA *s)
{
    MemoryRegion *sysmem = get_system_memory();
    ...
    sysbus_mmio_map(gicbusdev, 0, base_memmap[VIRT_GICA_DIST].base);
    sysbus_mmio_map(gicbusdev, 1, base_memmap[VIRT_GICA_REDIST].base);
...
static void create_apu(SigiVirtPartitionA *s)
{
    MemoryRegion *sysmem = get_system_memory();
    int i;
 ...
 */
static void create_ddr_memmap(SigiVirtPartitionA *s, int virt_mem)
{
...
static void create_unimp(SigiVirtPartitionA *s)
{
...
{
    SigiVirtPartitionA *s = SIGI_VIRT_PART_A(dev);
...
static Property sigi_virt_properties[] = {
    DEFINE_PROP_LINK("sigi-virt.ddr", SigiVirtPartitionA, cfg.mr_ddr, TYPE_MEMORY_REGION,
                     MemoryRegion *),
...
static const TypeInfo sigi_soc_info = {
    .name = TYPE_SIGI_VIRT_PART_A,
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(SigiVirtPartitionA),
