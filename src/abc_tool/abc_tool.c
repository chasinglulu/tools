...
/**
 * @brief Check if a NAND block is bad.
 *
 * @param offset The offset to check.
 *
 * Returns 1 if the block is bad, 0 otherwise. Returns a negative error code on failure.
 */
static int nand_is_bad(long long offset)
{
    __u64 blk_offset = offset;
    int ret = ioctl(abc_fd, MEMGETBADBLOCK, &blk_offset);

    if (ret < 0) {
        log_error("Failed to check bad block at offset %lld: %s", offset, strerror(errno));
        return -errno;
    }
 ...
 * @param size Pointer to store the device size.
 * @param blksz Pointer to store the block size.
 *
 * Returns 0 on success, a negative error code on failure.
 */
static int get_blkdev_info(const char *device,
                                unsigned int *size, unsigned int *blksz)
{
    if (size && ioctl(abc_fd, BLKGETSIZE64, size) == -1) {
        log_error("Could not get device size for %s: %s", device, strerror(errno));
        return -errno;
    }

    if (blksz && ioctl(abc_fd, BLKSSZGET, blksz) == -1) {
        log_error("Could not get block size for %s: %s", device, strerror(errno));
        return -errno;
    }
...
/**
 * @brief Get MTD device information.
 *
 * @param device The device path.
 * @param size Pointer to store the device size.
 * @param erasesize Pointer to store the erase block size.
 *
 * Returns MTD_NANDFLASH or MTD_NORFLASH on success, a negative error code on failure.
 */
int get_mtd_info(const char *device, unsigned int *size, unsigned int *erasesize)
{
    struct mtd_info_user mtd_info;

    if (ioctl(abc_fd, MEMGETINFO, &mtd_info) == 0) {
        *erasesize = mtd_info.erasesize;
        *size = (unsigned int)mtd_info.size;
        if (mtd_type_is_nand_user(&mtd_info)) {
            log_debug("Device %s is a NAND device, size: %u bytes, erase block size: %u bytes",
                     device, *size, *erasesize);
            return MTD_NANDFLASH;
        } else if (mtd_info.type == MTD_NORFLASH){
            log_debug("Device %s is a NOR device, size: %u bytes, erase block size: %u bytes",
                     device, *size, *erasesize);
            return MTD_NORFLASH;
        } else {
            log_error("Unsupported MTD type for device %s", device);
            return -ENOTSUP;
        }
    } else {
        log_error("Invaild MTD %s.", device);
        return -EINVAL;
    }
...
/**
 * @brief Process NAND metadata read/write.
 *
 * @param abc Pointer to the bootloader control structure.
 * @param size The size of the device.
 * @param erasesize The erase block size of the device.
 * @param is_write True for write operation, false for read operation.
 *
 * Returns the number of bytes transferred on success, a negative error code on failure.
 */
static int process_nand_metadata_rw(struct bootloader_control *abc,
                        uint32_t size, uint32_t erasesize,
                        bool is_write)
{
    long long current_offset;
    ssize_t bytes_transferred;
    const char *op_type = is_write ? "write" : "read";

    current_offset = offsetof(struct bootloader_message_ab, slot_suffix);
    while (current_offset < size) {
        if (nand_is_bad(current_offset)) {
            log_warn("Skipping bad block at offset %lld", current_offset);
            current_offset += erasesize;
            continue;
        }

        if (is_write)
            bytes_transferred = pwrite(abc_fd, abc, sizeof(*abc), current_offset);
        else
            bytes_transferred = pread(abc_fd, abc, sizeof(*abc), current_offset);

        if (bytes_transferred != sizeof(*abc)) {
            log_error("Could not %s A/B metadata at offset %lld: %s",
                        op_type, current_offset, strerror(errno));
            return -errno;
        } else {
            log_debug("Successfully %s A/B metadata at offset %lld", op_type, current_offset);
            return bytes_transferred;
        }
        current_offset += erasesize;
    }

    log_error("A/B metadata not found in NAND device");
    return -ENODEV;
}

/**
 * @brief Process metadata read/write.
 *
 * @param abc Pointer to the bootloader control structure.
 * @param is_write True for write operation, false for read operation.
 *
 * Returns the number of bytes transferred on success, a negative error code on failure.
 */
static int process_metadata_rw(struct bootloader_control *abc, bool is_write)
{
    ssize_t bytes_transferred;
    ...
    if (is_write)
        bytes_transferred = pwrite(abc_fd, abc, sizeof(*abc), abc_offset);
    else
        bytes_transferred = pread(abc_fd, abc, sizeof(*abc), abc_offset);

    if (bytes_transferred != sizeof(*abc)) {
        log_error("Could not %s A/B metadata: %s", strerror(errno));
        return -errno;
    } else {
        log_debug("Successfully %s A/B metadata", op_type);
        return bytes_transferred;
    }

    log_error("Not found A/B metadata in device");
    return -ENODEV;
}
...
static int abc_store(const char *device, struct bootloader_control *abc)
{
    ...
    if (S_ISBLK(device_stat.st_mode)) {
        uint32_t blksz;
        ret = get_blkdev_info(device, &size, &blksz);
        log_debug("Device %s size: %u bytes, block size: %u bytes", device, size, blksz);
    } else if (S_ISCHR(device_stat.st_mode)) {
        ret = get_mtd_info(device, &size, &erasesize);
        if (ret < 0)
            return -ENXIO;
        if (ret == MTD_NANDFLASH)
            is_nand = true;
    } else {
        log_error("Unsupported type of device %s", device);
        return -ENOTSUP;
    }

    if (is_nand) {
        bytes_written = process_nand_metadata_rw(abc, size, erasesize, true);
    } else {
        bytes_written = process_metadata_rw(abc, true);
    }

    if (bytes_written != sizeof(*abc)) {
        log_error("Could not write A/B metadata to device %s", device);
        return -EIO;
    }
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_generate_default_metadata(struct bootloader_control *abc)
{
    int i;
...
static int abc_load(const char *device)
{
    ...
    // Read A/B metadata
    if (is_nand) {
        bytes_read = process_nand_metadata_rw(&abc_metadata, size, erasesize, false);
    } else {
        bytes_read = process_metadata_rw(&abc_metadata, false);
    }

    if (bytes_read != sizeof(abc_metadata)) {
        log_error("Could not read A/B metadata from device %s", device);
        return -EIO;
    }

    ret = abc_validate(&abc_metadata);
    if (ret < 0) {
        log_error("Invaild A/B metadata within device %s", device);
        return ret;
    }
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_mark_successful(const char *device, int slot)
{
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_set_active_boot(const char *device, int slot)
{
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_set_unbootable(const char *device, int slot)
{
    struct slot_metadata *slotp = &abc_metadata.slot_info[slot];
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_check_bootable(const char *device, int slot)
{
    struct slot_metadata *slotp = &abc_metadata.slot_info[slot];
 ...
 *
 */
static int abc_check_bootup_status(const char *device, int slot)
{
    struct slot_metadata *slotp = &abc_metadata.slot_info[slot];
 ...
 * Returns 0 on success, a negative error code on failure.
 */
static int abc_get_suffix(const char *device, int slot)
{
...
static const char *get_action_name(action_t action)
{
    ...
    if (action >= GET_NUMBER_SLOTS && action <= GET_SUFFIX) {
...
static int abc_dump_slot_info(const char *device)
{
    int i;
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "hd:ncm:a:u:b:s:x:pgVv")) != -1) {
        ...
        switch (opt) {
            ...
            case 'p':
                ...
                break;
            case 'g':
                process_action(OPT_G, GENERATE_DEFAULT_METADATA, NULL);
                break;
            case 'V':
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        if (abc_load(devices[i]) < 0) {
            log_error("Failed to load A/B metadata from device %s", devices[i]);
            close(abc_fd);
            abc_fd = -1;
        } else {
            device_loaded = true;
    ...
    if (!device_loaded) {
        log_error("Failed to load A/B metadata from all specified devices");
        return EXIT_FAILURE;
    }
    ...
    for (int i = 0; i < action_list.action_count; i++) {
        ...
        switch (action_type) {
            ...
            case GET_CURRENT_SLOT:
                ...
                break;
            case MARK_BOOT_SUCCESSFUL:
                if (abc_mark_successful(devices[dev_idx], slot) < 0)
                    goto out;
                break;
            case SET_ACTIVE_BOOT_SLOT:
                if (abc_set_active_boot(devices[dev_idx], slot) < 0)
                    goto out;
                break;
            case SET_SLOT_AS_UNBOOTABLE:
                if (abc_set_unbootable(devices[dev_idx], slot) < 0)
                    goto out;
                break;
            case IS_SLOT_BOOTABLE:
                if (abc_check_bootable(devices[dev_idx], slot) < 0)
                    return EXIT_FAILURE;
                break;
            case IS_SLOT_MARKED_SUCCESSFUL:
                if (abc_check_bootup_status(devices[dev_idx], slot) < 0)
                    return EXIT_FAILURE;
                break;
            case GET_SUFFIX:
                if (abc_get_suffix(devices[dev_idx], slot) < 0)
                    return EXIT_FAILURE;
                break;
            case DUMP_SLOT_INFO:
                if (abc_dump_slot_info(devices[dev_idx]) < 0)
                    goto out;
                break;
            case GENERATE_DEFAULT_METADATA:
                 if (abc_generate_default_metadata(&abc_metadata) < 0)
                    goto out;
                break;
        }
    }
    ...
    if (metadata_changed) {
        ...
        if (current_checksum != initial_checksum) {
            ...
            if (abc_store(devices[dev_idx], &abc_metadata) < 0) {
                log_error("Failed to store A/B metadata");
                ret = EXIT_FAILURE;
            }
        } else {
